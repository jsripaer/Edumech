...
class RigidBody(System):
    '''A rigid body is a system that do not has motion between its particles.
    ...
    def _rotation(self, dt):
        '''
        NOTE: Up to now this library only supports 2D imitation, 
        positive direction of omiga and other quantity defined with 
        ...
        omiga = np.sqrt(v_t*v_t) / np.sqrt(r_pos*r_pos)
